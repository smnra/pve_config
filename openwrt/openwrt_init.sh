...
echo 'dns过滤 去广告 '
opkg install luci-app-dnsfilter



echo 'wifi-dog 广告投放 '
# https://blog.csdn.net/weixin_43614963/article/details/124964898
...
config host
    option mac '58:FB:84:2E:BF:03'
    option ip '192.168.10.11'
    option name 'lenovo-pc'
    option dns '1'
...
config host
    option name 'MagicBox2'
    option ip '192.168.10.207'
    option mac '38:B1:DB:64:4A:48'
    option dns '1'

config host
    option name 'yi-jia-Ace'
    option ip '192.168.10.208'
    option mac '7E:05:15:08:67:CF'
    option dns '1'
...
config redirect
    option dest 'lan'
    option target 'DNAT'
    option name 'home-pc_rdp'
    option src 'wan'
    option src_dport '9010'
...
config redirect
    option dest 'lan'
    option target 'DNAT'
    option name 'home-pc_ssh'
    option src 'wan'
    option src_dport '9011'
    option dest_ip '192.168.10.10'
    option dest_port '22'
...
config redirect
    option dest 'lan'
    option target 'DNAT'
    option name 'fnOS_webdav'
    option src 'wan'
    option src_dport '8015'
    option dest_ip '192.168.10.5'
...
config redirect
    option dest 'lan'
    option target 'DNAT'
    option name 'smnra-pc_vnc'
    option src 'wan'
    option src_dport '10020'
...
echo "config wechatpush 'config'
        option enable '1'
        option sleeptime '300'
        option debuglevel '1'
        option thread_num '3'
        option jsonpath '/usr/share/wechatpush/api/wxpusher.json'
        option wxpusher_apptoken 'AT_qMjyXykGQVAAa2ovltZN9lW9qSQmLUdf'
        option wxpusher_uids 'UID_P0LJszwDZMpuKHe2Cy7VY31leCIv'
        option device_name 'pve_openwrt'
        option oui_data '2'
        option get_ipv4_mode '1'
        list login_notification 'web_login_failed'
        option login_max_num '3'
        list device_notification 'online'
        list lite_enable 'device'
        option mac_filtering_mode_1 'block'
        option cpu_threshold_duration '90'
        option cpu_notification_delay '3600'
        option login_disturb '1'
        list login_ip_white_list '192.168.10.1'
        option login_log_enable '1'
        option passive_mode '1'
        option defaultSortColumn 'ip'
        list cpu_notification 'temp'
        option temperature_threshold '90'
        option ipv4_interface 'pppoe-wan'
        option soc_code 'pve'
        option server_host '192.168.10.2'
        option server_port '22'
" > /etc/config/wechatpush


echo "启动 wechatpush"
/etc/init.d/wechatpush start
...
######################################################################################################################
echo "\ndnsfilter 去广告  /etc/config/dnsfilter   ############################################################################"
echo "config dnsfilter
        option cron_mode '1'
        option time_update '4'
        option flash '1'
        option enable '1'
        option block_ios '1'
        option block_cnshort '0'
        option safe_search '1'
        list url 'https://adguardteam.github.io/AdGuardSDNSFilter/Filters/filter.txt'
        list url 'https://smnra.github.io/pve_config/noad/ruler.txt'
" > /etc/config/dnsfilter

/etc/init.d/dnsfilter enable
...
uci set ddns.my_ipv6_ddns.password='e91Jg5zA5ogXwNiQCDej1ETVk18GQU'
...
config service 'my_ipv6_ddns'
    ...
    option password 'e91Jg5zA5ogXwNiQCDej1ETVk18GQU'
    option ip_source 'network'
...
dynv6_ddns_password_ipv4=e91Jg5zA5ogXwNiQCDej1ETVk18GQU
dynv6_ddns_password_ipv6=e91Jg5zA5ogXwNiQCDej1ETVk18GQU
...
config config_subscribe
	option enabled '1'
	option name 'tg'
	option address 'https://ghfast.top/https://raw.githubusercontent.com/free18/v2ray/refs/heads/main/v.txt'
	option sub_ua 'clash.meta'
	option sub_convert '0'
